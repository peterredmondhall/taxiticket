...
import com.github.thiagolocatelli.paymill.Paymill;
...
import com.github.thiagolocatelli.paymill.model.APIObject;
import com.github.thiagolocatelli.paymill.utils.BASE64EncoderStream;
...
public abstract class APIResource extends APIObject {
	...
	protected static String singleClassURL(Class<?> clazz) {
		return String.format("%s/%s/%s", Paymill.getApiBase(), Paymill.VERSION, className(clazz));
	}
...
//			}
	...
	private static Map<String, String> flattenParams(Map<String, Object> params)
		...
		for (Map.Entry<String, Object> entry : params.entrySet()) {
			...
			if(value instanceof List<?>) {
			    ...
			    throw new InvalidRequestException("invalid_string_value","You cannot set '"+key+"' to an empty string. "+
	...
	private static APIResponse makeURLConnectionRequest(
		...
		try {
			...
			return new APIResponse(rCode, rBody, headers);
	...
	protected static <T> T _request(APIResource.RequestMethod method,
		...
		if ((Paymill.apiKey == null || Paymill.apiKey.length() == 0)
				&& (apiKey == null || apiKey.length() == 0)) {
			throw new AuthenticationException("api_key_missing",
					"No API key provided. (HINT: set your API key using 'Paymill.apiKey = <API-KEY>'. "
							+ "You can generate API keys from the Paymill web interface. ");
		}
		...
		try {
			...
			throw new InvalidRequestException("invalid_encoding", "Unable to encode parameters to "
					+ CHARSET);
		}
	...
	private static void handleAPIError(String rBody, int rCode)
			...
			TransactionErrorException, APIException,
			PreConditionFailedException, ResourceNotFoundException {
		Error error = GSON.fromJson(rBody, Error.class);
		switch (rCode) {
		case 400:
			throw new InvalidRequestException(error.error,
					error.errorDescription);
		case 401:
			throw new AuthenticationException(error.error,
					error.errorDescription);
		case 403:
			throw new TransactionErrorException(error.error,
					error.errorDescription);
		case 404:
			throw new ResourceNotFoundException(error.error,
					error.errorDescription);
		case 412:
			throw new PreConditionFailedException(error.error,
					error.errorDescription);
		default:
			throw new APIException(error.errorDescription, null);
		}
	}
